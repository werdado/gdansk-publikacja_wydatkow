...
Workflow [`.github/workflows/pages.yml`](.github/workflows/pages.yml) uruchamia się po każdym pushu do `master` oraz ręcznie (`workflow_dispatch`). Przy pierwszej konfiguracji poczekaj na zakończenie workflow po pushu albo uruchom go z karty Actions, aby utworzył gałąź `gh_pages`. Następnie w Settings → Pages ustaw źródło publikacji na „Deploy from a branch”, wybierz gałąź `gh_pages` i katalog `/(root)`, zapisz ustawienia i uruchom workflow ponownie. Każde uruchomienie najpierw wykonuje `npm ci`, testy jednostkowe, build i testy Chromium, a dopiero potem publikuje zawartość `dist/` w gałęzi `gh_pages`.
